...
class AlertManager:
    ...
    def _format_alert_message(self, target: str, changes: List[Dict], scan_dir: str) -> str:
        ...
        lines = []
        lines.append(f"🔍 ReconMaster Alert - {target}")
        lines.append(f"Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        lines.append(f"Scan Directory: {scan_dir}")
        lines.append("")
        lines.append(f"📊 Total Changes Detected: {len(changes)}")
        lines.append("")
        ...
        if low:
            lines.append("ℹ️ LOW PRIORITY:")
            for change in low:
                lines.append(f"  • {change['description']}")
            lines.append("")
        
        lines.append("---")
        lines.append(f"Generated by ReconMaster Monitoring System v4.0.0-Titan")
    ...
    def _send_email_alert(self, target: str, message: str):
        ...
        try:
            ...
            msg["Subject"] = f"🔍 ReconMaster Alert - {target}"
    ...
    def _send_slack_alert(self, target: str, message: str):
        ...
        try:
            ...
            # Format for Slack
            payload = {
                "text": f"🔍 *ReconMaster Alert - {target}*",
                "blocks": [
                    {
                        "type": "section",
                        "text": {
                            "type": "mrkdwn",
                            "text": message.replace("🚨", ":rotating_light:").replace("⚠️", ":warning:").replace("📊", ":bar_chart:")
                        }
                    }
                ]
            }
