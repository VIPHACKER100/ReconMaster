...
class ReconMaster:
    def __init__(self, target, output_dir, threads=20, wordlist=None, timeout=30):
        ...
        # Set timeout handler where supported (SIGALRM isn't available on Windows)
        if hasattr(signal, "SIGALRM"):
            try:
                signal.signal(signal.SIGALRM, self._timeout_handler)
            except Exception:
                logger.debug("Unable to set SIGALRM handler on this platform")
        else:
            logger.debug("SIGALRM not available; timeout signals disabled on this platform")

    def _timeout_handler(self, signum, frame):
        raise TimeoutError("Operation timed out")

    def create_dirs(self):
        # JS/url extraction is handled later in `crawl_endpoints` when files exist.
        dirs = [
            self.output_dir,
    ...
    async def _run_cmd_async(self, cmd, timeout=None):
        ...
        try:
            ...
            # If the command appears to require a shell (contains pipes/redirection),
            # fall back to the old shell-based runner. Otherwise prefer safe_run
            # executed in a thread to avoid shell=True and improve cross-platform behavior.
            shell_chars = ["|", ">", "<", "&", ";", "$", "*", "`"]
            if isinstance(cmd, str) and any(ch in cmd for ch in shell_chars):
                ...
                # Use utils.safe_run in a thread to run without shell when possible
                loop = asyncio.get_running_loop()
                stdout, stderr, rc = await loop.run_in_executor(
                    None, safe_run, cmd, timeout
                )
                # safe_run already returns decoded text
    ...
    async def passive_subdomain_enum(self):
        ...
        # Run tools concurrently but capture stdout and write files via Python

        async def run_and_write(cmd, out_path, timeout=None):
            stdout, stderr, rc = await self._run_cmd_async(cmd, timeout=timeout)
            try:
                ...
                logger.error(f"Failed to write output for {cmd}: {e}")
            return out_path

        tasks = [
            run_and_write(f"subfinder -d {self.target}", subfinder_output),
            run_and_write(f"assetfinder --subs-only {self.target}", assetfinder_output),
            run_and_write(f"amass enum -passive -d {self.target}", amass_output, timeout=300),
        ]

        # Run all enumeration tools concurrently
        await asyncio.gather(*tasks)

        # Combine results efficiently (cross-platform)
    ...
    async def active_subdomain_enum(self):
        ...
        # Get wordlist line count (pure-Python)
        try:
            total_lines = 0
            with open(self.wordlist, "r", encoding="utf-8", errors="ignore") as wl:
                for _ in wl:
                    total_lines += 1
            chunks = (total_lines // chunk_size) + 1 if total_lines > 0 else 10
        except Exception:
            chunks = 10
        ...
        for i in range(chunks):
            ...
            # Create chunk files using pure-Python slicing
            try:
                with open(
                    self.wordlist, "r", encoding="utf-8", errors="ignore"
                ) as wl, open(chunk_file, "w", encoding="utf-8") as cf:
                    for lineno, line in enumerate(wl, start=1):
                        if lineno < start:
                            continue
                        if lineno > end:
                            break
        ...
        async def process_chunk(chunk_file, chunk_num):
            ...
            cmd = f"ffuf -u http://FUZZ.{self.target} -w {chunk_file} -of json -s -t 50 -rate 100"
            stdout, stderr, rc = await self._run_cmd_async(cmd, timeout=300)
            # Write stdout (JSON) to the expected output file for downstream processing
            try:
                ...
                logger.error(f"Failed to write ffuf chunk output: {e}")
            return output_file

        # Process chunks in parallel
        tasks = [
            process_chunk(chunk_file, i) for i, chunk_file in enumerate(chunk_files)
        ]
        ...
        # Clean up chunk files (pure-Python)
        try:
            for fname in os.listdir(ffuf_output_dir):
                if fname.startswith("chunk_") and fname.endswith(".txt"):
                    try:
                        os.remove(os.path.join(ffuf_output_dir, fname))
                    except Exception:
                        pass
        except FileNotFoundError:
            pass
    ...
    async def resolve_live_domains(self):
        ...
        # Run httpx with optimized settings; capture stdout and write file via Python
        cmd = (
            f"httpx -l {all_subdomains} -status-code -title -tech-detect -follow-redirects "
            f"-silent -rate-limit 150 -threads {self.threads} -timeout 10"
        )
        stdout, stderr, rc = await self._run_cmd_async(cmd, timeout=600)
        try:
            os.makedirs(os.path.dirname(live_domains_file), exist_ok=True)
            with open(live_domains_file, "w", encoding="utf-8") as f:
                if stdout:
                    f.write(stdout + "\n")
        except Exception as e:
            logger.error(f"Failed to write httpx output: {e}")
    ...
    async def take_screenshots(self):
        ...
        if not self.live_domains:
            logger.warning(
                "No live domains to screenshot. Run resolve_live_domains first."
            )
            return
        ...
        chunks = [
            domains[i : i + chunk_size] for i in range(0, len(domains), chunk_size)
        ]
    ...
    async def scan_for_takeovers(self):
        ...
        # Run subzy with concurrency; capture stdout and write results via Python
        cmd = f"subzy run --targets {all_subdomains} --concurrency {self.threads} --timeout 30"
        stdout, stderr, rc = await self._run_cmd_async(cmd, timeout=600)
        try:
            os.makedirs(os.path.dirname(takeovers_file), exist_ok=True)
            with open(takeovers_file, "w", encoding="utf-8") as f:
                if stdout:
                    f.write(stdout + "\n")
        except Exception as e:
            logger.error(f"Failed to write subzy output: {e}")
        ...
        try:
            ...
            if self.takeovers:
                logger.info(
                    f"Found {len(self.takeovers)} potential subdomain takeovers!"
                )
            else:
    ...
    async def crawl_endpoints(self):
        ...
        if not self.live_domains:
            logger.warning(
                "No live domains for crawling. Run resolve_live_domains first."
            )
            return
        ...
        # Run optimized katana; capture stdout and write file via Python
        cmd = f"katana -list {live_domains_file} -jc -silent -concurrency {self.threads} -timeout 20 -rate-limit 100"
        stdout, stderr, rc = await self._run_cmd_async(cmd, timeout=1200)
        try:
            ...
            logger.error(f"Failed to write katana output: {e}")

        # Extract JS files efficiently (pure-Python)
        try:
        ...
        try:
            ...
            batches = [
                js_urls[i : i + batch_size] for i in range(0, len(js_urls), batch_size)
            ]
    ...
    async def directory_bruteforce(self):
        ...
        logger.info(
            f"Selected {len(sample_domains)} domains for directory brute forcing"
        )
        ...
        async def brute_domain(domain):
            ...
            cmd = (
                f"ffuf -u {domain}/FUZZ -w {wordlist} -mc 200,204,301,302,307,401,403 "
                f"-of json -s -t 40 -rate 100 -c"
            )
            stdout, stderr, rc = await self._run_cmd_async(cmd, timeout=300)
            # Write output file from stdout
            try:
                os.makedirs(os.path.dirname(output_file), exist_ok=True)
                with open(output_file, "w", encoding="utf-8") as f:
                    if stdout:
            ...
            try:
                with open(output_file, "r") as f:
                    data = json.load(f)
                    return [
                        (domain, result.get("url", ""), result.get("status", 0))
        ...
        if interesting_dirs:
            interesting_dirs_file = os.path.join(
                self.output_dir, "endpoints", "interesting_dirs.txt"
            )
    ...
    async def find_parameters(self):
        ...
        filtered_urls_file = os.path.join(
            self.output_dir, "endpoints", "param_candidate_urls.txt"
        )

        # Filter promising URLs using Python regex instead of grep
        try:
            import re

            pattern = re.compile(
                r"\.(php|aspx|jsp|do|cgi|pl|py)([?#]|$)|"
                r"(search|query|find|login|register|admin|dashboard|api|get|list|view)",
                re.IGNORECASE,
            )
            with open(endpoints_file, "r", encoding="utf-8") as src, open(
                filtered_urls_file, "w", encoding="utf-8"
            ) as dst:
                for line in src:
        ...
        try:
            ...
            async def find_params(url):
                temp_file = os.path.join(
                    self.output_dir, "params", f"params_{hash(url) % 10000}.txt"
                )
                cmd = f"arjun -u {url} --passive -t 5 --silent"
                async with self.rate_limiter:
                    stdout, stderr, rc = await self._run_cmd_async(cmd, timeout=60)
                # Write arjun output into temp_file for downstream processing
                try:
            ...
            # Combine results (pure-Python merge + dedupe)
            params_set = set()
            ...
            try:
                ...
                with open(params_file, "w", encoding="utf-8") as out:
                    for p in sorted(params_set):
                        out.write(p + "\n")
            except Exception as e:
                logger.error(f"Failed to write params file: {e}")
    ...
    async def check_broken_links(self):
        ...
        sample_domains_file = os.path.join(
            self.output_dir, "subdomains", "sample_live_domains.txt"
        )
        ...
        # First crawl for external links: prefer aiohttp parsing, fallback to shell pipeline
        href_re = re.compile(r'href=["\']([^"\']+)["\']', re.IGNORECASE)
        for domain in sample_domains:
            # Use aiohttp when available for cross-platform parsing
            if _HAVE_AIOHTTP:
                try:
                    async with aiohttp.ClientSession() as session:
                        try:
                            async with session.get(domain, timeout=30) as resp:
                                try:
                                    text = await resp.text()
                                except Exception:
                                    text = ""
                        except Exception:
                            # Skip domains that fail to respond
                            continue
                except Exception:
                    continue
                ...
                for match in href_re.findall(text):
                    ...
                    if link.startswith("/"):
                        try:
                            ...
                            pass
                    if link.startswith("http") and self.target not in link:
                        urls_to_check.append(link)
            else:
                # Fallback: run the shell pipeline (less portable)
                try:
        ...
        # If aiohttp isn't available, skip link checking
        if not _HAVE_AIOHTTP:
            logger.warning("aiohttp not installed; skipping broken link HTTP checks")
            self.broken_links = []
            return
    ...
    async def port_scan(self):
        ...
        for domain in self.live_domains:
            ...
            try:
                ...
                if stdout:
                    ...
                    ip_prefix = ".".join(
                        ip.split(".")[:2]
                    )  # Use first two octets as prefix
        ...
        async def scan_host(host_info):
            host, ip = host_info
            output_file = os.path.join(
                self.output_dir, "reports", f"{host.replace('.', '_')}_nmap.txt"
            )

            # Optimize nmap scan: first do a quick scan of common ports
            quick_cmd = f"nmap -T4 -F -oN {output_file}.quick {host}"
            await self._run_cmd_async(quick_cmd, timeout=120)

            # Then do a more thorough scan of top ports
            full_cmd = f"nmap -p- -T4 --top-ports 1000 -oN {output_file} {host}"
            await self._run_cmd_async(full_cmd, timeout=300)

            # Combine results (pure-Python): append quick output into final output and remove temp
            try:
                quick_path = f"{output_file}.quick"
                if os.path.exists(quick_path):
                    with open(
                        quick_path, "r", encoding="utf-8", errors="ignore"
                    ) as qf, open(output_file, "a", encoding="utf-8") as of:
                        of.write(qf.read())
                    try:
