...
class ReconMaster:
    ...
    async def passive_subdomain_enum(self):
        ...
        tasks = [
            self._run_cmd_async(
                f"subfinder -d {self.target} -o {subfinder_output} -silent"
            ),
            self._run_cmd_async(
                f"assetfinder --subs-only {self.target} > {assetfinder_output}"
            ),
            self._run_cmd_async(
                f"amass enum -passive -d {self.target} -o {amass_output}", timeout=300
    ...
    async def resolve_live_domains(self):
        ...
        # First ensure we have the combined sub
    ...
    async def find_parameters(self):
        ...
        # Filter promising URLs using Python regex (cross-platform)
        try:
            pattern = re.compile(
                r"\.(php|aspx|jsp|do|cgi|pl|py)([?#]|$)|(?:search|query|find|login|register|admin|dashboard|api|get|list|view)",
                re.IGNORECASE,
            )
            ...
            logger.warning("Endpoints file not found for parameter filtering")
            return
        ...
        try:
            ...
            # Write unique sorted parameters
            try:
    ...
    async def check_broken_links(self):
        ...
        # First crawl for external links (use curl then parse with regex)
        href_re = re.compile(r'href=["\']([^"\']+)["\']')
        for domain in sample_domains:
            stdout, stderr, rc = safe_run(["curl", "-s", "-L", domain])
            if stdout:
                for match in href_re.findall(stdout):
                    link = match.strip()
    ...
    async def port_scan(self):
        ...
        async def scan_host(host_info):
            ...
            # Optimize nmap scan: first

            # Optimize nmap scan: first check common ports
            cmd = f"nmap -p 80,443,22,21,8080,53 {host} -oN {output_file} -sV"
            await self._run_cmd_async(cmd, timeout=120)  # 2 minutes timeout
        ...
        logger.info("Port scanning completed. Results saved.")

        # Optionally, you can process scan results here if needed
        for result in scan_results:
            logger.info(f"Scan results saved to {result}")

    async def generate_report(self):
        """Generate a comprehensive report of the reconnaissance"""
        logger.info("Generating report")

        report_file = os.path.join(self.output_dir, "reports", "recon_report.txt")
        ...
        with open(report_file, "w") as f:
            ...
            f.write("Interesting Directories:\n")
            interesting_dirs_file = os.path.join(
                self.output_dir, "endpoints", "interesting_dirs.txt"
            )
            if os.path.exists(interesting_dirs_file):
                with open(interesting_dirs_file, "r") as dir_file:
            ...
            f.write("Tech Stack Information:\n")
            for domain, techs in self.tech_stack.items():
                f.write(f"{domain}: {', '.join(techs)}\n")
            f.write("\n")

            f.write("Port Scan Results:\n")
            for domain in self.live_domains:
                port_scan_file = os.path.join(
                    self.output_dir, "reports", f"{domain.replace('.', '_')}_nmap.txt"
                )
                if os.path.exists(port_scan_file):
                    f.write(f"Results for {domain}:\n")
                    with open(port_scan_file, "r") as scan_file:
                        f.write(scan_file.read())
                    f.write("\n")
                else:
                    f.write(f"No scan results for {domain}.\n")
...
async def main():
    parser = argparse.ArgumentParser(
        description="ReconMaster - A comprehensive reconnaissance tool"
    )
    ...
    parser.add_argument(
        "-t", "--threads", type=int, default=20, help="Number of threads to use"
    )
    parser.add_argument(
        "-w", "--wordlist", help="Path to custom wordlist for subdomain enumeration"
    )
    parser.add_argument(
        "--timeout", type=int, default=30, help="Timeout for commands in seconds"
    )

    args = parser.parse_args()

    recon_master = ReconMaster(
        args.target, args.output, args.threads, args.wordlist, args.timeout
    )

    await recon_master.passive_subdomain_enum()
    await recon_master.active_subdomain_enum()
    await recon_master.resolve_live_domains()
    await recon_master.take_screenshots()
    await recon_master.scan_for_takeovers()
    await recon_master.crawl_endpoints()
    await recon_master.directory_bruteforce()
    await recon_master.find_parameters()
    await recon_master.check_broken_links()
    await recon_master.port_scan()
    await recon_master.generate_report()
